#!/bin/bash
LIST_NODES=$(sudo /usr/local/sbin/rabbitmqctl cluster_status|sed -n '3p'| sed -n '/running_nodes,\[.*\]\},/p' | sed  -r 's/\[|\]|\{|\}|running_nodes,//g')
...
function nodes_detect {
    ...
    do
       local VHOST_LIST=$(sudo /usr/local/sbin/rabbitmqctl -n ${node} list_vhosts|sed '1d'|sed '/...done./d')
       ...
       do
           ...
           if [[ $type_detect -eq 1 ]]; then
               local list_queue=$(sudo /usr/local/sbin/rabbitmqctl -n ${node} -p ${vhost} list_queues | sed '1d'|sed '/...done./d'|awk '{print $1}')
           ...
           if [[ $type_detect -eq 2 ]]; then
               local list_exchange=$(sudo /usr/local/sbin/rabbitmqctl -n ${node} -p ${vhost} list_exchanges | sed '1d'|sed '/...done./d'|awk '{print $1}')
